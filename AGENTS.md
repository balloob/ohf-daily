...
- In `Just shipped`, stable Home Assistant and ESPHome releases link to the matching official website release post when evidenced in the local content store, including the monthly post for patch releases. Preserve the GitHub release URL as provenance and fallback; do not invent a post URL or use a stable post for a beta.
...
- The collector also refreshes the configured public Luma iCalendar feeds for community meetups. These listings live in `communityMeetups`, survive editorial resolution, and appear chronologically as normal event rows in Upcoming. Multiple meetups starting on the same date collapse into one expandable count row for that date. Publisher-confirmed relationships in `data/community-event-groups.json` place related meetups inside the parent event row instead: the September 23 Lisbon meetup belongs to OHF Summit, and the November 7 Community Day meetups belong to Community Day. Show only meetup names and links inside these lists, with no times, locations or timezone note. Do not infer relationships from overlapping dates alone, turn each meetup into an article, or add duplicate individual rows to `events`. Retain overlapping and multi-day meetups, deduplicate shared events across feeds, and exclude cancelled or expired entries. A feed failure blocks collection rather than silently publishing a partial count. To refresh only these listings in an existing edition, use `npm run refresh:meetups -- --date YYYY-MM-DD`.
