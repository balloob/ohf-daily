...
- The collector also refreshes the configured public Luma iCalendar feeds for community meetups. These listings live in `communityMeetups`, survive editorial resolution, and appear as one expandable count row in the release calendar, with individual dates, places and links inside. Do not turn each meetup into an article or add duplicate individual rows to `events`. Retain overlapping and multi-day meetups, deduplicate shared events across feeds, and exclude cancelled or expired entries. A feed failure blocks collection rather than silently publishing a partial count. To refresh only these listings in an existing edition, use `npm run refresh:meetups -- --date YYYY-MM-DD`.
