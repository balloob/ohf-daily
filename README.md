...
Home Assistant releases are calculated for the first Wednesday of each month, with beta starting seven days earlier. ESPHome follows two weeks after Home Assistant and its beta starts fourteen days before release. Change `release_cycles` if the publication calendar changes. `release_preview_sources` maps those products to their official preview material; Home Assistant uses its RC preview site and ESPHome uses its next-version site. Music Assistant can be added when its official preview URL and release cycle are known.

On a configured stable release day, the official release source is ingested even when it was first published before the edition's 24-hour reporting window, and the resulting release article is the front-page lead. Its described features are treated as release contents and reported confidently without draft-status or missing-artifact caveats. Matching landed stable-release metadata can support exact availability and artifact claims. Betas, prereleases, release candidates, and patch releases remain discretionary unless their verified contents are independently newsworthy.
