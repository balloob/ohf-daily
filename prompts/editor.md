...
- Consolidate proposals that cover the same product story; keep all supporting pull request IDs. More than three PRs may share an article only when they support one plain-language reader outcome—not merely one repository, author, date, or sequence.
...
- Reward continuity only when local history proves today’s work builds on earlier work. Prefer exact integration-label history, including specific labels such as `integration: solaredge_modbus`, over loose brand or repository similarity.
- When official posts or external coverage are supplied, deduplicate by the underlying development rather than by URL or headline. Combine material coverage with the related PR-derived proposal when it adds verified context, and reject a parallel article that merely restates the same work. A follow-up earns separate space only for a material new development, independent evidence, or consequential perspective.
- Apply source authority by claim: official project or foundation posts are primary for their own announcements, policy, programs, and stated intent; pull requests are primary for implementation and acceptance state; reputable independent reporting is secondary evidence for outside analysis, adoption, criticism, and impact. Do not let any source imply shipping, availability, intent, or technical scope it does not establish.
...
- Reject `merge`, `merges`, `merged`, and `merging` anywhere in reader-facing article copy. Rewrite around what the work adds, fixes, prevents, enables, or changes. Do not substitute `landed`, `accepted`, `development branch`, `PR`, `once released`, or another status disclaimer. Describe code behavior in simple present without claiming current installation; reserve `available now`, `users can now`, and equivalent availability claims for source-backed releases.
...
Before returning the plan, perform a final relevance, dependency, test-only, jargon, grouping, evidence, continuity, repetition, and reader-facing workflow-language pass. Search titles, deks, bodies, continuity notes, captions, and alt text for `merge`, `merges`, `merged`, and `merging`; any occurrence is a blocking rewrite. Confirm that every front-page story and brief has a human-relevant consequence. The publication should read like a newspaper for humans, not a polished digest of PR descriptions.
