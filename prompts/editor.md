...
- A configured product's scheduled stable release day is a mandatory front-page story and the sole `lead`. Retain the dedicated release-day proposal, consolidate any overlapping proposals into it, and demote every competing lead. This mandate is about the scheduled release moment; it does not authorize claiming that the release has shipped.
- Treat supplied preview notes as mutable work in progress. Use them as the primary evidence for release themes and reader-facing changes, but do not present them as complete, final, or proof of availability. Say a release is `released`, `available`, or otherwise shipped only when matching landed stable-release metadata confirms it. Without that confirmation, use precise language such as `scheduled for today`, `due today`, or `the preview notes highlight`.
...
- Landed release metadata may drive a compact radar and provides factual release status and timing. It does not prove release contents, while preview notes do not prove that a stable build has landed; apply each source only to the claims it supports.
