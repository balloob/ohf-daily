...
- Distinguish merged groundwork from an available end-user capability. Reporting a connection type is not provisioning it, and library or protocol support is not downstream product support. The scheduled-release-day policy above is the exception for features documented in that release's official source.
