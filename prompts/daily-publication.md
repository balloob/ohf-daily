...
Scheduling check: verify the stored next trigger as an absolute timestamp converted to `America/New_York`, not merely a displayed hour or a timezone label in the recurrence text. The desktop scheduler ignored a timezone-qualified start in the September 10 repair; an explicit UTC start produced the correct next trigger. At each run, calculate the next New York midnight and adjust this same automation through the scheduling tool if its UTC hour needs to change for daylight saving. Preserve its prompt, destination and notification preferences, and verify the resulting next timestamp after any adjustment. Small scheduler dispatch delays are acceptable; a several-hour timezone shift is not.
